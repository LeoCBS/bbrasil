...
export default async function ClientsPage({ searchParams }: Props) {
  ...
  const [result, profiles] = await Promise.all([getPaginatedClients({ search, status, page: parsePageParam(params?.page), pageSize }), getProfiles()]);
  const profilesMap = new Map(profiles.map(p => [p.id, p.name || p.email]));
