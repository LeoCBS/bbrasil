...
export function OrderForm({
  ...
  const [selectedSalesperson, setSelectedSalesperson] = useState<Profile | null>(
    order?.client_salesperson_id ? profiles.find(p => p.id === order.client_salesperson_id) || null : null
  );
  ...
  // Observation
  const [observation, setObservation] = useState<string>(order?.observation || '');
  ...
  async function handleSubmitForm(formData: FormData) {
    if (!selectedClient) {
      setSubmitError('Selecione um cliente para o pedido.');
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        setSuccessMessage(order ? 'Pedido atualizado com sucesso.' : 'Pedido criado com sucesso.');
        ...
        console.error('Falha ao salvar o pedido:', reason);
        setSubmitError(reason instanceof Error ? reason.message : 'Não foi possível salvar o pedido.');
      }
    });
  }
  ...
  return (
    <form ref={formRef} onSubmit={onSubmit} className="grid gap-4">
      ...
      {order ? <input type="hidden" name="id" value={order.id.toString()} /> : null}
      {(userId || order?.user_id) && <input type="hidden" name="user_id" value={userId || order?.user_id || ''} />}
      {(userName || order?.user_name) && <input type="hidden" name="user_name" value={userName || order?.user_name || ''} />}
      {(userEmail || order?.user_email) && <input type="hidden" name="user_email" value={userEmail || order?.user_email || ''} />}
      ...
      <SubmitButton className="w-full" pendingLabel={order ? 'Salvando...' : 'Criando...'} forcePending={isPending}>
