...
import { currentUnitStorageKey } from "@/components/site/add-to-quote-button";
...
export function UnitSelector({ selectedUnit, units }: { selectedUnit?: Unit; units: Unit[] }) {
  ...
  function selectUnit(unit: Unit) { setIsSelecting(true); window.localStorage.setItem(currentUnitStorageKey, unit.id); document.cookie = `bbrasil_selected_unit_id=${encodeURIComponent(unit.id)}; Path=/; Max-Age=31536000; SameSite=Lax`; window.dispatchEvent(new CustomEvent("bbrasil:quote-cart-updated", { detail: { open: false } })); router.replace(`/?unidade=${encodeURIComponent(unit.id)}`); }
